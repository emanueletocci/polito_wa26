		...
		db.all(sql, (err, rows) => {
			if (err) {
				...
				if (filter) {
					...
					else reject({ error: "The specified filter is not available" });
...
const createFilm = (film) => {
	...
	if (film.rating <= 0) film.rating = null;
...
const updateFilm = (id, film) => {
	return new Promise((resolve, reject) => {
		...
		db.run(
			sql,
			[film.title, film.favorite, film.watchDate, film.rating, id],
			function (err) {
				if (err) {
					reject(err);
				} else {
...
const updateFilmRating = (id, deltaRating) => {
