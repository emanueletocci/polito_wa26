...
import filmDao from './dao.mjs'; // module for accessing the films table in the DB
...
app.post('/api/films',
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ error: `Database error during the creation of new film: ${err}` }); 
    }
  }
);
...
app.put('/api/films/:id/favorite',
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ error: `Database error during the favorite update of film ${req.params.id}` });
    }
  }
);
...
app.post('/api/films/change-rating',
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ error: `Database error during the rating update of film ${req.params.id}` });
    }
  }
);
