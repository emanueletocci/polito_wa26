import dayjs from "dayjs";

function Film(id, title, isFavorite = false, watchDate, rating) {
	this.id = id;
	this.title = title;
	this.favorite = isFavorite;
	this.rating = rating;
	// saved as dayjs object only if watchDate is truthy
	this.watchDate = watchDate && dayjs(watchDate);

	this.toString = () => {
		return (
			`Id: ${this.id}, ` +
			`Title: ${this.title}, Favorite: ${this.favorite}, ` +
			`Watch date: ${this.formatWatchDate("MMMM D, YYYY")}, ` +
			`Score: ${this.formatRating()}`
		);
	};

	this.formatWatchDate = (format) => {
		return this.watchDate ? this.watchDate.format(format) : "<not defined>";
	};

	this.formatRating = () => {
		return this.rating ? this.rating : "<not defined>";
	};
}

function FilmLibrary() {
	this.list = [];

	this.addNewFilm = (film) => {
		if (!this.list.some((f) => f.id == film.id)) this.list.push(film);
		else throw new Error("Duplicated id");
	};

	this.print = () => {
		this.list.forEach((item) => console.log(item.toString()));
	};

	this.sortByDate = () => {
		const newArray = [...this.list];
		newArray.sort((d1, d2) => {
			if (!d1.watchDate)
				return 1; // null/empty watchDate d1 is the higher value
			else if (!d2.watchDate) return -1;
			return d1.watchDate.diff(d2.watchDate, "day");
		});
		return newArray;
	};

	/*
    this.deleteFilm = (id) => {
        const index = this.list.findIndex(film => film.id == id);
        if (index != -1){
            this.list.splice(index, 1);
        }
    }
    */

	/*
    this.deleteFilm = (id) => {
        const tempList = this.list.filter((film) => film.id != id);

        this.list = tempList;
    }
    */

	this.deleteFilm = (id) => {
		const tempList = this.list.filter((film) => {
			return film.id != id;
		});
		this.list = tempList;
	};

	/*
    this.resetWatchedFilms = () => {
        this.list.forEach((film) => film.watchDate = undefined);
    }
    */

    // the delete keyword remove the entire key-value entry from the js object
    // So the object should be recalculate by the engine 
	this.resetWatchedFilms = () => {
		this.list.forEach((film) => delete film.watchDate);
	};

    
    this.getRated = () => {
        const tempList = this.list.filter((f) => f.rating > 0);

        tempList.sort((f1, f2) => f2-f1 );
        return tempList;
    }
}

function main() {
	// Creating some film entries
	const f1 = new Film(1, "Pulp Fiction", true, "2023-03-10", 5);
	const f2 = new Film(2, "21 Grams", true, "2023-03-17", 4);
	const f3 = new Film(3, "Star Wars", false);
	const f4 = new Film(4, "Matrix");
	const f5 = new Film(5, "Shrek", false, "2023-03-21", 3);

	// Adding the films to the FilmLibrary
	const library = new FilmLibrary();
	library.addNewFilm(f1);
	library.addNewFilm(f2);
	library.addNewFilm(f3);
	library.addNewFilm(f4);
	library.addNewFilm(f5);

	// Print Films
	console.log("***** List of films *****");
	library.print();

	library.sortByDate();
	console.log("\n***** List of films sorted by watch date *****");
	library.print();

	library.deleteFilm(3);

	console.log("\n***** List after remove *****");
	library.print();

	library.resetWatchedFilms();
	console.log("\n***** List after reset *****");
    console.log(library);
	library.print();

    const ratedList = library.getRated();
    console.log("\n***** List after rating *****");
    ratedList.forEach(f => console.log(f.toString()));

	// Additional instruction to enable debug
	debugger;
}

main();
