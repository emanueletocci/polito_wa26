    ...
    <Table>
      ...
      <tbody>
        {films.map((film) => <FilmRow filmData={film} key={film.id} delete={props.delete}
           editFilm={props.editFilm} disabled={props.disabled} />)}
      </tbody>
    </Table>
  );
}
    ...
    <tr>
      ...
      <td className="text-center">
        ...
        <Form.Check type="checkbox" checked={props.filmData.favorite ? true : false}
          ...
          onChange={(event) => props.editFilm({...props.filmData, favorite: event.target.checked}, filterId) } />
      </td>
      <td>
        <small>{formatWatchDate(props.filmData.watchDate, 'MMMM D, YYYY')}</small>
      </td>
      <td>
        <Rating rating={props.filmData.rating} maxStars={5} disabled={props.disabled}
          editRating={(newRating) => { if (newRating!==props.filmData.rating) props.editFilm({ ...props.filmData, rating: newRating }, filterId)} } />
      </td>
      <td>
        <Button variant='danger' disabled={props.disabled}
          onClick={() => { props.delete(props.filmData.id, filterId) }} >
        ...
        <Button className="mx-2" variant='warning' disabled={props.disabled}
...
function Rating(props) {
  ...
  return [...Array(props.maxStars)].map((el, index) =>
    <i key={index} className={(index < props.rating) ? "bi bi-star-fill" : "bi bi-star"}
      style={{ color: props.disabled ? "gray" : "black" }}  
