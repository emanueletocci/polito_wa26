...
app.put('/api/films/:id', isLoggedIn, isTotp,
  ...
  async (req, res) => {
    ...
    const errors = validationResult(req).formatWith(errorFormatter); // format error message
    if (!errors.isEmpty()) {
      setTimeout(()=> res.status(422).json( errors.errors ), responseDelay); // error message is sent back as a json with the error info
      return;
    }
...
app.put('/api/films/:id/favorite', isLoggedIn,
  ...
  async (req, res) => {
    ...
    try {
      ...
      setTimeout(()=>res.json(result), responseDelay);
      return;
    } catch (err) {
      console.log(err);  // Logging errors is expecially useful while developing, to catch SQL errors etc.
      res.status(500).json({ error: 'Database error' });
