    ...
    <tr>
      ...
      <td className="text-center">
        ...
        <Form.Check type="checkbox" checked={props.filmData.favorite ? true : false}
          disabled={props.disabled} 
          onChange={(event) => props.setFilmFavorite(props.filmData.id, event.target.checked, filterId) } />
      </td>
      <td>
...
disabled={props.disabled}
          editRating={(newRating) => { if (newRating!==props.filmData.rating) props.setFilmRating(props.filmData.id, newRating, filterId )} } />
      </td>
      <td>
        <Button variant='danger' disabled={props.disabled || props.disableActions}
          onClick={() => { props.delete(props.filmData.id) }} >
          <i className='bi bi-trash'></i>
        </Button>
        <Button className="mx-2" variant='warning' disabled={props.disabled ||  props.disableActions}
