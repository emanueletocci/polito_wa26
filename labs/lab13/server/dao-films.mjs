...
const searchFilms = (user, searchString) => {
  return new Promise((resolve, reject) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    db.run(sql, [film.title, film.favorite, film.watchDate, film.rating, film.user], function (err) {
      if (err) {
        ...
        resolve(getFilm(film.user, this.lastID));
      }
    });
  });
};
...
const updateFilmRating = (user, id, deltaRating) => {
...
export default { listFilms, getFilm, createFilm, updateFilm, 
    updateFilmRating, deleteFilm, searchFilms };
